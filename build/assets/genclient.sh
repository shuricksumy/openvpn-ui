...
# backup  sed -i'.bak' "$ s/$/\/name=${1}/" $OVDIR/easy-rsa/pki/index.txt
sed -i'.bak' "$ s/$/\/name=${1}\/LocalIP=${2}/" $OVDIR/easy-rsa/pki/index.txt
...
echo "$(cat $OVDIR/client-template.txt)
<ca>
$CA
</ca>
<cert>
...
echo "Checkout $OVDIR/clients/$1.ovpn"
