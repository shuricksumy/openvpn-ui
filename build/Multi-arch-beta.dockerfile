...
RUN cd /go/src/github.com/shuricksumy/openvpn-ui
RUN git clone https://github.com/shuricksumy/openvpn-ui ./ && git checkout ${BRANCH}
...
RUN apt-get update && apt-get install -y curl bzip2 jq git wget openvpn iptables openssl wget ca-certificates iproute2 procps sqlite3
